...
# Rename the columns.
names(m) <- c("Date", "Cause of Death", "Death Rate")
m
...
nightingale_1 <- nightingale_1[order(nightingale_1$"Death Rate", decreasing = TRUE), ]
nightingale_2 <- nightingale_2[order(nightingale_2$"Death Rate", decreasing = TRUE), ]
m <- rbind(nightingale_1, nightingale_2)

coxcombs_1 <- ggplot(nightingale_1,
                   aes(x = factor(Date),
                       y = nightingale_1$"Death Rate",
